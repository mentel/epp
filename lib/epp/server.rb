module Epp #:nodoc:
  class Server
    ...
    attr_accessor :tag, :password, :server, :port, :lang, :services, :extensions, :version, :socket_options
    ...
    # * <tt>:socket_options (Hash)</tt> - Key-value pairs that are set directly on the ssl socket object. Defaults to {}
    def initialize(attributes = {})
      ...
      @socket_options = attributes[:socket_options] || {}
    ...
    def open_connection
      ...
      socket_context = OpenSSL::SSL::SSLContext.new
      @socket_options.each do |key, val|
        socket_context.send("#{key}=", val)
      end
      @socket = OpenSSL::SSL::SSLSocket.new(@connection, socket_context)
